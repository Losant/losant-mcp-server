...
| `timeSourcePath` | — | Payload path to a time value. Required when `timeSourceType: "payloadPath"`. Accepted formats: a numeric **milliseconds-since-epoch** value (≥ 1,000,000,000,000); a numeric **seconds-since-epoch** value (< 1,000,000,000,000 — auto-multiplied by 1000); `0` for current time; a negative number of milliseconds offset from now (e.g. `-5000` = 5 seconds in the past); an ISO 8601 string or any string parseable by `new Date()`; or a Losant LJSON date object `{ "$date": "..." }`. |
