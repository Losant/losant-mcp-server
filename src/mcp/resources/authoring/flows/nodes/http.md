...
| `timeoutTemplate` | Default `"30"`. Per-request timeout in seconds. Values above 30 are clamped to 30 at runtime — the stored value is not modified. |
...
- Setting a timeout above 30 seconds — clamped to 30 at runtime; the stored config value is unchanged.
...
| `errorIfFileExists` | Default `false`. When `true`, throws an error if the disk target file already exists. Omitting the field results in overwrite behavior. |
