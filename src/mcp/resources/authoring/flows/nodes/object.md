...
| `zipObject` | **Required** — JSON array of key names | — | Combine source (array of values) with `inputTemplate` (array of keys) to produce an object. Source must be an array. **Requires `destPath`** to persist result. |
