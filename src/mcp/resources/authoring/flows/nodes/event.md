...
| `timeSourcePath` | Payload path to a timestamp. Used when `timeSourceType: "payloadPath"`. Accepted formats: a numeric **milliseconds-since-epoch** value (≥ 1,000,000,000,000); a numeric **seconds-since-epoch** value (< 1,000,000,000,000 — auto-multiplied by 1000); `0` for current time; a negative number for an offset relative to now (ms); an ISO 8601 string or any string parseable by `new Date()`; or a Losant LJSON date object `{ "$date": "..." }`. If the value cannot be parsed, falls back to current time. |
