...
- **Filter by `key` to target specific severity tiers.** Use `"critical"` or `"error"` for alerting workflows and `"any"` only for audit/logging workflows that must capture everything.
