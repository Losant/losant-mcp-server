...
The File Trigger allows you to execute a flow when a file is created, modified, or deleted from a directory in your Application Files.
...
- **Renaming or moving a file fires the trigger twice** — once as a `delete` at the old path and once as a `create` at the new path. If your flow needs to handle renames atomically, track the two events via storage or a data table keyed on the file ID.
- **Scope the `key` path as narrowly as possible.** A trigger watching `"/"` fires on every file operation in the application. Narrow it to the directory your flow cares about to avoid unnecessary executions.
...
- **Use `data.action` to branch** on `"create"`, `"update"`, and `"delete"` in a single flow rather than creating separate triggers for each operation type.
